...
- **Plain pip + venv**: rejected for lacking a lockfile by default (reproducibility across the two development sessions this project explicitly runs — builder and teacher, per `CLAUDE.md` — depends on a locked, reproducible dependency set) and for the extra manual steps (`requirements.txt` + `requirements-dev.txt` maintenance) `uv`/Poetry both remove.
...
- Project hard rules this ADR formalizes (branch model, `pytest` green, conventional commits): `CLAUDE.md`
