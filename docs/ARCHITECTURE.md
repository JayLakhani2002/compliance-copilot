...
| **Bad citation** (`_validate_citations`, ADR-0014) | Every citation's `(regulation, anchor)` must be among the articles `retrieve_node` actually fetched (recitals never count), and its `quote` must match the cited excerpt — an exact normalised substring first, falling back to a high-floor `difflib` similarity only on a miss (`guards/quotes.py`'s `quote_matches`, ADR-0031) so cosmetic drift (punctuation, whitespace, a dropped marker) doesn't fail a genuine quote | `CitationError`, caught by `answer_node` and retried once with the failure fed back to the model (ADR-0015); still failing after that retry raises the hard error — no answer is returned, never a silently-repaired citation. A citation accepted only via the fuzzy fallback is logged as a `quote_fuzzy_match` guardrail event (score only) so Langfuse shows how often it fires. |
