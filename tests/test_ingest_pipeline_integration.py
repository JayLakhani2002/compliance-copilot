...
# DATABASE_URL — runs in CI's pgvector job, same skip convention as
# test_db_integration.py). Uses FakeEmbeddings (tests/fake_embeddings.py) so
# this costs no OpenAI money — that's what makes it safe to run in CI on
# every PR, unlike the RUN_NETWORK_TESTS=1-gated real-embedding test in
# test_search_real_embeddings_integration.py.
import os
...
if not DATABASE_URL:
    pytest.skip(
        "DATABASE_URL not set — skipping ingest pipeline integration tests",
...
def test_ingest_twice_second_run_embeds_zero(engine, fixture_regulations):
...
def test_dry_run_does_not_commit(engine, fixture_regulations):
    embeddings = FakeEmbeddings()
    with Session(engine) as session:
